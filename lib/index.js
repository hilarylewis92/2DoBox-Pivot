...
$('.todo-list').on('click', '.up-btn',  function() {
  ...
  // let importanceText = $(this).closest("importance-rating").text();
...
// Todo.prototype.upvote = function () {
...
function upvote (foundTodo) {
  foundTodo.importance = "test";
  localStorage.setItem('list', JSON.stringify(storage));
  $(".todo-list").text("");
  renderLocalStorageToPage();
}

function changeImportance(id, newImportance) {
  id = parseInt(id);

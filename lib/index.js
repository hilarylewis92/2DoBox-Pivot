...
function renderLocalStorageToPage(todo) {
...
$('.show-more-todos').on('click', function(){
  var allTodos = storage.slice(10,storage.length);
...
$('.list').on("click", ".critical-btn, .high-btn, .normal-btn, .low-btn, .none-btn", function () {
  ...
  if (whatDidIClick === "critical-btn") {
    ...
    $(".true").each(function () {
      $(this).children().children().addClass("completed");
...
//   $('.todo-list .todo').slice(0, 2).show();
//   if ($('.todo-list .todo').length == $('.todo-list .todo:visible').length) {
//     $('span ').hide();
//   }
// }
